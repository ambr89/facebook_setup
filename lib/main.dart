...
import 'package:facebook_setup_package/android.dart' as android_launcher_icons;
import 'package:facebook_setup_package/ios.dart' as ios_launcher_icons;
import 'package:facebook_setup_package/constants.dart' as constants;
import 'package:facebook_setup_package/custom_exceptions.dart';
import 'facebook_setup_package.dart';
...
Future<void> performUpdate(String context)async {
  var syncPath = await path;
  if (io.File(syncPath).exists())
    throw const NoInfoPlistFileFoundException(
        constants.errorIncorrectIconName);
  final updater = Updater();
  updater.updateIosApplicationIdFromConfig(context);
  // updater.updateAndroidApplicationIdFromConfig(context);
}